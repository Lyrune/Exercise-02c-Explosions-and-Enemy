...
onready var Asteroid_small = load("res://Asteroid/AsteroidSmall.tscn")
	...
	if health <= 0:
		...
		if Asteroid_Container != null:
			for s in small_asteroids:
				...
				var smalldir = randf()*2*PI
				var i = Vector2(0, randf()*small_speed).rotated(smalldir)
				Asteroid_Container.call_deferred("add_child", asteroid_small)
				asteroid_small.position = position + s.rotated(smalldir)
